function normalizeCvssInput(value) {
  return typeof value === 'string' ? value.trim() : '';
}

function extractCvssVersion(value) {
  return value.match(/^CVSS:(\d(?:\.\d)?)/i)?.[1] ?? null;
}

export function extractCvssDisplayText(value) {
...
export function getCvssReferenceHref(value) {
  const normalized = normalizeCvssInput(value);
